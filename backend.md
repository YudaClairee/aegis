...
5. Guardian no-response escalation:
   - menerima SOS otomatis dari missed safety check-in,
   - menyimpan trigger context bila dikirim frontend,
   - membedakan notifikasi no-response dari manual/sensor SOS.
6. Audio upload:
   - validasi file,
...
sequenceDiagram
    ...
    M->>M: Danger signal detected or Guardian safety check-in missed
    ...
    API->>FCM: Send SOS/no-response alert to family devices
...
- `TriggerType`
  - wajib mencakup `'manual' | 'keyword' | 'risk_engine' | 'notification_button' | 'no_response'`.
- `IncidentLocation`
- `SOSTriggerContext`
  - optional metadata untuk no-response escalation, misalnya `guardianStartedAt`, `lastCheckInAt`, `missedCheckInAt`, dan `escalationReason`.
- `AISummary`
- `Incident`
- `IncidentListItem`
- `LocationUpdate`
...
- `SOSTriggerSchema`
  - `triggerType` menerima `'no_response'` untuk SOS otomatis akibat missed safety check-in.
  - `triggerContext` optional object untuk menyimpan konteks Guardian/no-response tanpa memblokir trigger.
- `AudioUploadSchema`
...
packages/shared/src/constants/guardian.ts
```
...
- `ACCELEROMETER`
- `SAFETY_CHECKIN`
  - contoh default MVP: interval check-in 5 menit, grace period 60 detik, max missed check-in 1 sebelum SOS.
...
- `emergency_contacts`
- `incidents`
  - `trigger_type` wajib menerima `no_response`.
  - tambahkan `trigger_context JSONB DEFAULT '{}'::jsonb` untuk metadata safety check-in/no-response.
- `location_history`
- triggers:
  - `handle_new_user()`
  - `update_updated_at()`
...
Catatan Guardian/no-response:

- Untuk MVP, safety check-in timer dimiliki frontend; backend tidak perlu scheduler.
- Backend hanya perlu menerima `POST /api/sos/trigger` dengan `triggerType = 'no_response'` saat frontend mendeteksi user tidak merespons check-in.
- `trigger_context` di `incidents` dipakai untuk audit/debug, bukan untuk menentukan apakah SOS boleh dibuat.
...
sendNoResponseNotification(params) // boleh wrapper dari sendSOSNotification dengan copy berbeda
...
```ts
{
  type: 'sos_alert' | 'no_response_alert'
  incidentId: string
  victimUserId: string
  victimName: string
...
⚠️ SafeHer No-Response Alert
{victimName} tidak merespons Guardian check-in. Tap untuk melihat lokasi terakhir/live.
```
...
5. Create incident:
   ...
   - `risk_score`
   - `keywords_detected`
   - `trigger_context` optional JSONB.
6. Fetch accepted linked emergency contacts.
7. Send FCM alert async:
   - normal SOS copy untuk `manual`, `keyword`, `risk_engine`, `notification_button`,
   - no-response copy untuk `trigger_type = 'no_response'`.
...
- `no_response` tidak membutuhkan keyword/audio/risk score agar tetap bisa menangani kondisi freeze/tonic immobility.
- `trigger_context` tidak boleh berisi data sensitif; simpan hanya timestamp/status check-in yang membantu audit.
...
- Trigger accepts `triggerType = 'no_response'` without keywords/audio/risk score.
- `no_response` stores optional `trigger_context` and uses no-response notification copy.
...
- [ ] `triggerType = 'no_response'` membuat incident tanpa keyword/audio/risk score.
- [ ] Linked family receives no-response FCM copy saat missed check-in escalation.
...
- [ ] `no_response` trigger create incident dan memakai notification copy khusus missed check-in.
...
5. Untuk missed safety check-in, frontend trigger SOS dengan `triggerType: 'no_response'` dan optional `triggerContext`.
6. Push notification deep link:
   - SOS alert: `safeher://tracking/{incidentId}`
   - no-response alert: `safeher://tracking/{incidentId}`
   - AI update: `safeher://tracking/{incidentId}`
   - resolved/false alarm: `safeher://incidents/{incidentId}`
7. Family yang belum linked tidak boleh melihat incident/tracking.
8. SOS tetap dibuat walaupun belum ada family linked atau FCM gagal.
