...
3. Mengaktifkan **Guardian Mode**.
4. Mendeteksi risiko dari GPS, accelerometer, dan distress keywords.
5. Menampilkan countdown 5 detik sebelum auto-SOS.
6. Memicu SOS secara manual atau otomatis.
7. Mengirim lokasi terakhir ke backend.
8. Merekam audio lingkungan selama 15 detik setelah SOS.
9. Mengunggah audio untuk dianalisis AI.
10. Mengirim live location via Supabase Realtime.
11. Menampilkan live tracking untuk keluarga/kontak darurat.
12. Menampilkan riwayat dan detail insiden.
13. Menangani offline mode secara aman.
...
2. **Offline tolerant**: SOS, audio, dan lokasi harus bisa di-buffer saat offline.
3. **No secret in mobile**: hanya pakai public anon key dan public API URL.
4. **Minimal tap during emergency**: tombol SOS harus besar, jelas, dan cepat diakses.
5. **Explicit permissions**: jelaskan alasan akses lokasi, mikrofon, dan notifikasi dengan copy yang user-friendly.
6. **Non-blocking SOS**: setelah user trigger SOS, jangan tunggu audio/AI selesai untuk membuat incident. Incident harus dibuat dulu.
7. **Realtime state ≠ server state**:
   - TanStack Query untuk contacts, incidents, profile.
   - Zustand untuk Guardian Mode, risk score, sensor status, active SOS.
8. **Top-level background task**: semua `TaskManager.defineTask` wajib didefinisikan di top-level module, bukan di dalam component React.
...
2. Implement `detectAccelerometerAnomaly(x, y, z)`:
   - shake jika magnitude `> SHAKE_THRESHOLD * 9.81`,
   - drop jika magnitude `< DROP_THRESHOLD * 9.81`.
3. Cap total risk score di 100.
4. Return:
   - `totalScore`,
   - `level`,
   - `shouldTriggerCountdown`,
   - `signals`.
5. Trigger countdown jika `totalScore >= RISK_LEVELS.CRITICAL`.

**Acceptance criteria:**

- Unit test manual atau dev screen bisa menunjukkan risk score naik.
- Score critical membuka countdown sekali, tidak berulang-ulang selama modal aktif.
...
3. Dalam task handler:
   ...
   - hitung speed anomaly jika ada previous location,
...
5. Jika critical, panggil flow countdown.
...
3. Saat keyword terdeteksi:
   ...
   - update risk signal,
   - hitung total risk,
   - trigger countdown jika critical.
...
4. Jika `STOP_ACTION`:
   - stop Guardian.
5. Pastikan listener tidak didaftarkan berkali-kali.
...
1. Risk critical atau manual SOS membuka modal.
2. Timer berjalan 5 detik.
3. Jika user cancel:
   - tutup modal,
   - reset temporary risk trigger,
   - Guardian tetap aktif.
4. Jika timeout:
   - panggil `triggerSOS('risk_engine' | 'keyword' | 'manual')`.
...
- Auto SOS dari countdown memakai payload risk/keywords.
...
6. Risk engine.
7. Guardian UI realtime.

**Output:** Guardian bisa aktif, GPS/sensor berjalan, risk score berubah.
...
4. Manual SOS button.
5. Notification action SOS/Stop.
6. SOS active screen.
...
- [ ] Risk critical membuka countdown.
...
- [ ] Offline verification lulus minimal untuk SOS trigger dan audio queue.
...
8. Format audio file yang paling aman untuk Gemini/backend:
   - `.m4a`,
   - `.wav`,
   - `.webm`.
...
4. Geofencing safe/unsafe zones.
5. Route anomaly detection yang lebih akurat.
6. Family app role khusus.
7. Share live tracking link via WhatsApp/SMS.
8. Local encrypted storage untuk offline audio.
9. E2E test dengan Detox/Maestro.
10. EAS build pipeline.
